...
final public class MultipartFormData {
    enum FormDataError: Error {
        case endCodingStringToDataFailed(_ String: String)
        var errorDescription: String? {
            switch self {
            case .endCodingStringToDataFailed(let string):
                return "Could not encode \"\(string)\" as .utf data."
            }
        }
    }
    ...
    static let CRLF = "\r\n"
    // boundary is unique string use to mark the boundaries betweent different data part in a mutipart/form-data request
    let boundary: String
    ...
    public convenience init() {
        self.init(boundary: "yolo.boundary.\(UUID().uuidString)")
    }
    ...
    private func encode(string: String) throws -> Data {
        guard let encodedData = string.data(using: .utf8, allowLossyConversion: false) else {
            throw FormDataError.endCodingStringToDataFailed(string)
        }
